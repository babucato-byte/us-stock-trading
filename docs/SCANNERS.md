...
`broker/`, `brokers/`, `execution/`, `live_pilot/`, `market_data/candidate_store.py`
를 import 하지 않습니다. 이는 규칙이 아니라 구조로 보장되며
`tests/test_scanner_trading_isolation.py` 가 소스 트리를 직접 검사합니다.
...
`SCANNER_ANALYTICS_DIR` 는 주문 후보 저장소(`KIS_CANDIDATE_DIR`)와
**의도적으로 별개의 환경변수**입니다 (§10). 서로를 가리키게 설정하면 안 됩니다.
