...
## 추가 갱신(2026-07-28, Codex 독립 재검증 `PASS_WITH_CONDITIONS`)

Codex가 CODEX-039/040/041 배선(커밋 `ae2b0fd`/`fc20574`)을 독립 재검증해 `PASS_WITH_CONDITIONS`를
부여하며(`docs/autonomous/CODEX_REVIEW.md`, 커밋 `ebce9d0`) 다음을 필수 후속 조건으로 명시했다:
"제한적 live review 전에 operator TBD, kill-switch 절차, reconciliation runbook을 사람이
검토한다." 즉 이 문서에 나열된 10개 TBD 항목 — 특히 #2(실제 연결 계정), #3(주문당 최대 금액),
#4(허용 종목 범위), #5(허용 거래 시간대), #6~8(계좌/주문 실측), #9(운영자 승인), #10(롤백 담당자)
— 은 여전히 운영자가 직접 검토·기입해야 하며, 코드/테스트가 Codex 재검증을 통과했다는 사실이
이 항목들을 대신 채우지 않는다. 같은 날 사용자가 "운영자 제한값 없는 자동 운영 구조"를 별도로
지시했으나(`cash_usage_percent` 기본값을 90으로 자동화하는 등), #4(허용 종목 범위)를 "시스템
자동 선별"로 대체하려면 실제 시장 전체 후보 자동 선별 기능이 먼저 구현돼야 하고 아직 구현되지
않았으므로, 이 문서의 #4 권장값과 승인 필요 여부는 이번 갱신에서 바꾸지 않았다(상세는
`docs/autonomous/DECISION_LOG.md`의 "자동 운영 구조 전환 착수" 섹션 결정 4 참고).
