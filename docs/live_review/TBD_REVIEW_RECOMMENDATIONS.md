...
| 3 | §3 리스크 한도 | 주문당 최대 금액(절대 금액, USD) | **$500 ~ $1,000** (제한적 파일럿 규모 제안) | 현재 `risk_config.py`에는 비중 기반 한도(`MAX_POSITION_RATE=0.10`)만 있고 절대 금액 상한이 없음. "제한적 소액 실거래"라는 이번 사이클의 목표(사용자 지시서 반복 언급)에 맞춰, 계좌 자본과 무관하게 단일 주문이 넘지 못할 절대 상한을 두는 것이 안전. 예: 계좌가 예상보다 커도 1건 주문이 과도해지지 않도록 하는 하드캡 | **높음** — 이 값이 없으면 계좌 자본이 커질수록 `MAX_POSITION_RATE=10%`의 절대 금액도 함께 커져, "소액 파일럿" 의도와 무관하게 큰 금액이 나갈 수 있음. 코드에 이 하드캡을 강제하는 로직이 아직 없다는 점도 별도 위험(문서 값과 코드 동작 불일치) | **예** — 실제 배분 가능한 자본 규모는 운영자만 알고, 리스크 감내 수준도 운영자 판단 사항 |
| 4 | §3 리스크 한도 | 허용 종목 범위(심볼 allow-list) | **초기 파일럿은 고정된 소규모 allow-list(예: 상위 유동성 대형주 5~10종목)로 시작**, 이후 안정성 확인되면 기존 동적 스캐너(`scalping_watchlist`, `MIN_PRICE=5`/`MAX_PRICE=500`/`MIN_AVERAGE_DOLLAR_VOLUME=2000만`/`MIN_LIQUIDITY_SCORE=20` 기준)로 확대 | `risk_config.py`/`account_risk.py`에는 심볼 allow-list/블랙리스트가 없음. `config/scalping_watchlist_config.py`의 가격·거래량·유동성 필터가 사실상의 동적 허용 범위 역할을 하지만, 이는 "무엇이 스캔 대상인가"를 결정할 뿐 "실거래를 허용할 종목"을 별도로 좁히는 장치는 아님 | **중간** — 필터를 통과한 종목이라도 뉴스/실적 발표 등 이벤트로 인한 이상 변동성에 노출될 수 있음. 초단타 파일럿 초기에는 스캐너가 걸러내는 넓은 유니버스보다 사람이 잘 아는 소수 종목으로 좁히는 편이 리스크가 낮음 | **예** — 어떤 종목을 파일럿 대상으로 신뢰할지는 운영 판단이며, 규제/세금/유동성 이해도에 따라 운영자별로 다를 수 있음 |
