...
class TestResultStore:
    ...
    def test_analytics_dir_is_not_the_candidate_dir(self, monkeypatch, tmp_path):
        ...
        Asserted against the CONTRACT rather than against the trading
        candidate store module, because that module does not exist on
        every branch this framework has to run on -- and the requirement
        does. Importing it unconditionally would make the test's
        availability depend on the branch, which is the opposite of what
        an invariant this important needs.

        The two facts checked here hold everywhere:

          * the two stores are configured by DIFFERENT environment
            variables, so pointing one cannot move the other, and
          * the analytics store resolves under its own variable, not to
            the repository root where `order_candidates.csv` lives.

        `test_a_scanner_run_writes_nothing_into_the_candidate_store` in
        tests/test_scanner_trading_isolation.py covers the runtime half.
        """
        candidate_dir_env = "KIS_CANDIDATE_DIR"
        assert result_store.ANALYTICS_DIR_ENV != candidate_dir_env

        analytics = tmp_path / "analytics"
        monkeypatch.setenv(candidate_dir_env, str(tmp_path / "order-candidates"))
        monkeypatch.setenv(result_store.ANALYTICS_DIR_ENV, str(analytics))
        resolved = result_store.analytics_dir()

        assert resolved == analytics
        assert resolved != Path(os.environ[candidate_dir_env])
        assert not str(resolved).startswith(str(tmp_path / "order-candidates"))

        # Where the trading candidate store IS available, hold it to the
        # stronger statement too: the resolved directories differ.
        try:
    ...
    def test_the_analytics_store_never_uses_the_order_candidate_filename(self):
        """A scanner must not produce a file the order path would read.

        `order_candidates.csv` is the name `daily_candidate_scanner.py`
        writes and the order path consumes. Nothing the analytics store
        writes may share it, whatever directory it ends up in.
        """
        assert result_store.SIGNALS_SUBDIR != "order_candidates.csv"
        written = {
            result_store.signals_path("2026-08-12").name,
            result_store.performance_path("2026-08-12").name,
            result_store.runs_path("2026-08-12").name,
        }
