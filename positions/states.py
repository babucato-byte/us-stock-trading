...
#: The shares left by some route this system did not drive -- a manual
#: sale, a broker action, a transfer. Terminal, and deliberately NOT
#: CLOSED: CLOSED asserts this system sold and knows the exit price,
#: and reusing it here would put an invented exit into the record.
#: Reached only through reconciliation/external_close.py, which
#: requires the broker to be flat with no open order, no unresolved
#: order, no unresolved exit intent, and no exit in flight.
...
TERMINAL_STATES = {CLOSED, REJECTED, CANCELLED, EXPIRED, EXTERNALLY_CLOSED}
...
TRANSITIONS = {
    ...
    FILLED: {STOP_ACTIVE, EXTERNALLY_CLOSED},
    STOP_ACTIVE: {TARGET_1_ACTIVE, EXIT_SUBMITTED, MANUAL_REVIEW, RECOVERY_REQUIRED,
                  EXTERNALLY_CLOSED},
    TARGET_1_ACTIVE: {PARTIAL_EXIT_SUBMITTED, EXIT_SUBMITTED, MANUAL_REVIEW,
                      RECOVERY_REQUIRED, EXTERNALLY_CLOSED},
    ...
    PARTIAL_EXITED: {TRAILING, EXIT_SUBMITTED, MANUAL_REVIEW, RECOVERY_REQUIRED,
                     EXTERNALLY_CLOSED},
    TRAILING: {EXIT_SUBMITTED, MANUAL_REVIEW, RECOVERY_REQUIRED, EXTERNALLY_CLOSED},
